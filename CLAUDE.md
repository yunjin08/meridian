...
| `chat.ts` | `POST /api/chat` | Session | Assistant loop: four alert write tools, five tax write tools, and four read tools executed server-side, two portfolio-watchlist write tools applied by the browser |
...
| `utils/chat-tools.ts` | — | — | Chat tool schemas, read-tool, alert-tool, and tax-tool executors, compact result formatting |
